...
router.post('/', (req, res) => {
    ...
    db.query('INSERT INTO orders (price, date, user_id) VALUES ($1, $2, $3) RETURNING *', [price, date, user_id])
        .then(data => res.json(data.rows))
        .catch(err => console.error(err))
})
...
router.put('/:id', (req, res) => {
    const { id } = req.params
    const { price, date, user_id } = req.body
    db.query('UPDATE orders SET price=$1, date=$2, user_id=$3 WHERE id=$4 RETURNING *', [price, date, user_id, id])
        .then(data => res.json(data.rows))
        .catch(err => console.error(err))
})
